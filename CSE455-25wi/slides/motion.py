...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        # b = -It[y-w : y+w+1, x-w : x+w+1].flatten()[:, np.newaxis]
        ...
        # vx, vy = np.linalg.inv(A.T @ A) @ A.T @ b[:, 0]
...
def iterative_lucas_kanade(img1, img2, keypoints, window_size=9, num_iters=7, g=None):
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        G = np.array([[np.sum(Ix_window ** 2), np.sum(Ix_window * Iy_window)], [np.sum(Ix_window * Iy_window), np.sum(Iy_window ** 2)]])
        ...
        for k in range(num_iters):
            ...
            ### YOUR CODE HERE
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            I1_window = img1[y1-w:y1+w+1, x1-w:x1+w+1]
            I2_window = img2[y2-w:y2+w+1, x2-w:x2+w+1]
            temporal_diff = (I2_window - I1_window).flatten()
            bk = np.array([np.sum(temporal_diff * Ix_window), np.sum(temporal_diff * Iy_window)])
            vk = -np.dot(Ginv, bk)
...
def pyramid_lucas_kanade(
    ...
    for L in range(level, -1, -1):
        ...
        d = iterative_lucas_kanade(I_L, J_L, p_L, window_size, num_iters, g)
        # deals with not scaling in last iteration
        if L > 0:
          g = scale * (g + d)
        # else:
        #   g = g + d
...
def compute_error(patch1, patch2):
    ...
    error = np.mean((((patch1 - patch1.mean()) / patch1.std()) - 
                     ((patch2 - patch2.mean()) / patch2.std()))**2)
...
def IoU(bbox1, bbox2):
    ...
    if y2 >= y1 + h1 or y2 + h2 <= y1 or x2 >= x1 + w1 or x2 + w2 <= x1:
      intersection = 0
    else:
      intersection = (min(y1 + h1, y2 + h2) - max(y2, y1)) * (min(x1 + w1, x2 + w2) - max(x2, x1))
